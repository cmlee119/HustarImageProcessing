...
extern "C" __declspec(dllexport) bool DllMainInit()
{
    ...
    m_square_points.push_back(cv::Point2f(0.f, 0.f));
    m_square_points.push_back(cv::Point2f(m_marker_image_side_length - 1.f, 0.f));
    m_square_points.push_back(cv::Point2f(m_marker_image_side_length - 1.f, m_marker_image_side_length - 1.f));
    m_square_points.push_back(cv::Point2f(0, m_marker_image_side_length - 1.f));
    ...
    *pInputVideo >> input_image;
    ...
    for (int i = 0; i < detectedMarkers.size(); i++)
    {
        ...
        if (!identify(bitMatrix, marker_id, rotation))
        {
            ...
            int sumx = 0, sumy = 0;
            for (int j = 0; j < 4; j++) {
                const int ix = static_cast<int>(m[j].x);
                const int iy = static_cast<int>(m[j].y);

                putText(input_image, to_string(j + 1), Point(ix, iy), QT_FONT_NORMAL, 1, Scalar(255, 0, 0), 1, 1);
                sumx += ix;
                sumy += iy;
            }
            ...
            markerTransform.x = sumx / 4.f;// translation_vector.data[0, 0];
            markerTransform.y = sumy / 4.f;// translation_vector.data[0, 1];
...
extern "C" __declspec(dllexport) void DllMainStartLoop()
{
    while (true) {
        if (true == bClose)
            break;
...
extern "C" __declspec(dllexport) bool DllMainGetRawImageBytes(IN OUT unsigned char* data, OUT MarkerTransform** vecMarkerTransform, OUT int* itemCount)
{
    if (true == bFirst)
    {
        return false;
    ...
    *vecMarkerTransform = new MarkerTransform[m_vecMarkerTransform.size()];
    ...
    cv::Mat argb_img;
    cv::cvtColor(input_image, argb_img, cv::COLOR_RGB2BGRA);
...
extern "C" __declspec(dllexport) void DllMainCloseLoop()
{
    bClose = true;
}

extern "C" __declspec(dllexport) bool DllMainGetFrameSize(OUT int* pWidth, OUT int* pHeight)
{
    if (pInputVideo == nullptr)
    {
        return false;
    ...
    *pWidth = static_cast<int>(pInputVideo->get(CAP_PROP_FRAME_WIDTH));
    *pHeight = static_cast<int>(pInputVideo->get(CAP_PROP_FRAME_HEIGHT));
